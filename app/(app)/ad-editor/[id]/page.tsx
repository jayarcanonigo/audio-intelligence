...
export default function AdEditorPage() {
  ...
  // Backend/legacy values (NEW, SAVED, new, completed, pending...) all collapse to the
  // two statuses SelectedSegments actually renders: "NEW" (highlighted, unsaved) and "SAVED".
  const normalizeStatus = (status: any): "NEW" | "SAVED" => {
    const value = String(status || "").trim().toUpperCase();
    if (value === "SAVED" || value === "COMPLETED") return "SAVED";
    return "NEW";
  };
  ...
  const loadLogs = useCallback(
    async (opts: { silent?: boolean } = {}) => {
      ...
      try {
        ...
        const hour = broadcastHour === "all" ? undefined : Number(broadcastHour);
        ...
        if (ads && ads.length > 0) {
          const loaded = ads.map((ad: any) => {
            const matchedLogs = list.filter(
              (log: any) => log.start_time === ad.start_time && log.end_time === ad.end_time
            );
            ...
            return {
              ...
              detection_key: ad.detection_key || null,
              // IMPORTANT: preserve the real DB status so already-saved ads don't relight as "NEW"
              status: normalizeStatus(ad.status),
              advertisement: true,
              segment_type: "advertisement",
              segmentIds: matchedLogs.map((log: any) => log.id),
            };
          });

          setResults(loaded);
          setDisabledLogs(loaded.flatMap((item: any) => (item.segmentIds.length ? item.segmentIds : [item.id])));
        }
  ...
  useEffect(() => {
    async function loadHours() {
      try {
        ...
        setHours(data);
        if (data.length > 0) setBroadcastHour(String(data[0]));
      } catch (error) {
        console.error("Failed loading segment hours", error);
      }
    }
    if (projectId) loadHours();
  }, [projectId]);
  ...
  const handleReprocessAds = async () => {
    try {
      ...
      console.log("================================");
      console.log("REPROCESS START", "Project ID:", projectId, "Hour:", hour);
      console.log("================================");

      const result = await reprocessAdvertisements(projectId, hour);

      console.log("REPROCESS RAW RESULT:", result);

      // Handle possible response structures
      const data = result?.data ?? result?.result ?? result;
      console.log("REPROCESS DATA:", data);

      const detected = Number(data?.advertisements ?? 0);
      const created = Number(data?.created ?? 0);
      const skipped = Number(data?.skipped ?? 0);
      console.log("Detected:", detected, "Created:", created, "Skipped:", skipped);

      if (created > 0) {
        toast.success(`🔄 ${created} new advertisement${created === 1 ? "" : "s"} reprocessed`);
      } else if (detected > 0) {
        toast.info(`Found ${detected} advertisement${detected === 1 ? "" : "s"}, but none were newly created.`);
      } else {
        toast.warning("No advertisements detected.");
      }
      ...
      console.error("================================");
      toast.error(error?.message || "Failed to reprocess advertisements");
    }
  };
  ...
  const handleDownloadExcel = async () => {
    ...
    const formatLength = (start?: string, end?: string) => {
      const totalSeconds = getSeconds(end) - getSeconds(start);
    ...
    const exportData = [...results]
      .sort((a, b) => getSeconds(a.start) - getSeconds(b.start))
      .map((r) => ({
        "Start HH:MM:SS": r.start || "00:00:00",
        "End time HH:MM:SS": r.end || "00:00:00",
        "ACTUAL LENGTH": formatLength(r.start, r.end),
        "BRAND": r.brand_name || "",
        "COPYLINE": r.text || "",
      }));
    ...
    if (exportData.length === 0) {
      toast.warning("No advertisements to export");
      return;
    }
    ...
    try {
      ...
      a.href = url;
      a.download = "advertisement_report.xlsx";
  ...
  function handleUpdateSegment(
    id: number,
    data: { text: string; start: string; end: string; brand_name: string; status: "NEW" | "SAVED" }
  ) {
    console.log("UPDATE SEGMENT ID:", id, "UPDATE DATA:", data);
    ...
    setResults((prev) =>
      prev.map((item) =>
        item.id === id
          ? { ...item, text: data.text, start: data.start, end: data.end, brand_name: data.brand_name, status: data.status }
          : item
      )
    );
  }
  ...
  const handleAddRange = async () => {
    ...
    const newSegment = {
      ...
      status: "NEW",
      segment_type: "new",
      detection_key: `manual-${projectId}-${range[0].start_time}-${range[range.length - 1].end_time}`,
    };
  ...
  async function handleSaveAllSegments() {
    try {
      ...
      // DELETE EXISTING ADS FOR THIS HOUR
      await deleteAdvertisementsByProjectHour(projectId, Number(broadcastHour));

      // CREATE CURRENT ADS
      for (const segment of results) {
        // DETECTION KEY
        let detectionKey = segment.detection_key || null;

        // If this segment came from transcript logs, create a stable detection key.
        if (!detectionKey && segment.segmentIds && segment.segmentIds.length > 0) {
          const ids = segment.segmentIds;
          const startSegmentId = ids[0];
          const endSegmentId = ids[ids.length - 1];
          detectionKey = `project-${projectId}-segments-${startSegmentId}-${endSegmentId}`;
        }

        // Single segment fallback
        if (!detectionKey && segment.id) {
          detectionKey = `project-${projectId}-segments-${segment.id}-${segment.id}`;
        }
  ...
  const handleCenterLastCompleted = () => {
    ...
    setTimeout(() => {
      const element = document.getElementById(`segment-${lastSavedId}`);
  ...
  const handleAddSingle = (row: any) => {
    const newSegment = {
      ...
      status: "NEW",
      segment_type: "advertisement",
      detection_key: row.detection_key || `manual-${projectId}-${row.start_time}-${row.end_time}`,
    };
  ...
  const handleTimeUpdate = () => {
    if (audioRef.current) setCurrentAudioTime(audioRef.current.currentTime);
  };
    ...
    <div className="p-3 md:p-6 space-y-6">
      ...
      <div className="bg-white rounded-xl shadow p-4 md:p-5">
        <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
          {/* Project Info */}
          <div>
            <h1 className="text-xl md:text-2xl font-bold text-gray-800">🎧 Ad Editor</h1>
            <p className="mt-1 text-sm text-gray-500 break-words">
              Project: <span className="font-medium text-gray-700">{projectName || `Project #${projectId}`}</span>
            </p>
          </div>
          ...
          <div className="w-full md:w-auto">
            ...
            <div className="hidden md:block">
              <div className="mb-2 text-xs font-semibold uppercase tracking-wide text-gray-500">Broadcast Hour</div>
        ...
        <div className="pb-40">
            ...
            <div className="flex gap-2">
              <button
                ...
                className={`flex-1 rounded-lg py-2 text-sm font-semibold transition ${
                  mobileTab === "logs" ? "bg-blue-600 text-white" : "bg-white text-gray-600 border"
                }`}
              >
                Live Logs
              </button>
              <button
                ...
                className={`flex-1 rounded-lg py-2 text-sm font-semibold transition ${
                  mobileTab === "segments" ? "bg-blue-600 text-white" : "bg-white text-gray-600 border"
                }`}
              >
          ...
          {mobileTab === "logs" ? (
            ...
            <div className="bg-white rounded-xl shadow p-4">
              <h2 className="mb-4 font-semibold">Live Logs</h2>
              <LiveLogs
                logs={logs}
            ...
            <>
              ...
              <div className="mb-3 flex rounded-xl bg-gray-100 p-1">
                <button
                  onClick={() => setMobileTab("logs")}
                  className={`flex-1 rounded-lg py-2 text-sm font-semibold transition ${
                    mobileTab === "logs" ? "bg-blue-600 text-white shadow" : "text-gray-600"
                  }`}
                >
                  📝 Logs
                </button>
                <button
                  onClick={() => setMobileTab("segments")}
                  className={`flex-1 rounded-lg py-2 text-sm font-semibold transition ${
                    mobileTab === "segments" ? "bg-blue-600 text-white shadow" : "text-gray-600"
                  }`}
                >
                  🎧 Segments ({results.length})
                </button>
