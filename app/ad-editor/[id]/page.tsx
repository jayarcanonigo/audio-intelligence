...
const ffmpegRef = useRef<any>(null);
  ...
  try {
    ...
    if (ads && ads.length > 0) {
      ...
      // ======================================
      // NO SAVED ADS
      // LOAD DETECTED ADVERTISEMENT SEGMENTS
      // FROM LOGS
      // ======================================
  ...
  const handleDownloadExcel = async () => {
    const exportData = [...results]
      .sort((a, b) => {
        const aStart = toSeconds(a.start) ?? Number.MAX_SAFE_INTEGER;
        const bStart = toSeconds(b.start) ?? Number.MAX_SAFE_INTEGER;
...
async function handleSaveAllSegments() {
  ...
  try {
    ...
    // DELETE OLD ADS
    await deleteAdvertisementsByProject(
      projectId
    );
...
const handleRemove = (id: number) => {
    ...
    setDisabledLogs(
        updated.flatMap(
            (item:any) => item.segmentIds ?? [item.id]
        )
    );
...
const handleAddSingle = (row: any) => {
  ...
  const exists = results.some(
    (item) =>
      item.segmentIds?.includes(row.id) ||
      item.id === row.id
  );
